...
inline void Saturate(DoubleArray SignDist, char *id, Domain &Dm, int nx, int ny, int nz, int rank, double SW)
{
    double GlobalNumber = 1.f;
    double LocalNumber=0.f;
    int Nx = nx;
...
//        }
       ...
       // if (0 == Dm->rank()) {
        ...
        cout << "sum of solid sites=" << sum << endl;
        int singlesphere = 6152;
        int twospheres = 2*6152;
        int diff = twospheres - sum;
        cout << "Difference between sphere sizes=" << diff << endl;
