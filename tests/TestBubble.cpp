...
int main(int argc, char **argv)
{
  ...
  { // Limit scope so variables that contain communicators will free before MPI_Finialize
    ...
    auto tau   = color_db->getScalar<double>( "tau");  
    auto alpha = color_db->getScalar<double>( "alpha" );
    auto beta  = color_db->getScalar<double>( "beta" );
    auto phi_s = color_db->getScalar<double>( "phi_s" );
    auto Fx    = color_db->getVector<double>( "F" )[0];
    auto Fy    = color_db->getVector<double>( "F" )[1];
    auto Fz    = color_db->getVector<double>( "F" )[2];
    auto Restart = color_db->getScalar<bool>( "Restart" );
    auto pBC   = color_db->getScalar<double>( "pBC" );
    auto din   = color_db->getScalar<double>( "din" );
    auto dout  = color_db->getScalar<double>( "dout" );
    auto timestepMax = color_db->getScalar<int>( "timestepMax" );
    auto interval = color_db->getScalar<int>( "interval" );
    auto tol   = color_db->getScalar<double>( "tol" );
    auto das   = color_db->getScalar<double>( "das" );
    auto dbs   = color_db->getScalar<double>( "dab" );

    // Read variables from Domain
    auto size = domain_db->getVector<int>( "n" );
    auto L = domain_db->getVector<double>( "L" );
    auto nproc = domain_db->getVector<int>( "nproc" );
    auto nspheres = domain_db->getScalar<int>( "nspheres" );
    int Nx = size[0];
    int Ny = size[1];
    int Nz = size[2];
    int nprocx = nproc[0];
...
//    char value;
    ...
    for (int bubbleCount=0; bubbleCount<4; bubbleCount++){
      ...
      while (timestep < timestepMax && relError > CaThresh) {
            ...
            // Run the analysis
            //analysis.run2( timestep, *Averages, Phi, Pressure, Velocity, fq, Aq, Bq, Den, Np);
            analysis.run( timestep, *Averages, Phi, Pressure, Velocity, fq, Den );
        // allow initial ramp-up to get closer to steady state
                    if ( timestep > analysis_interval && (timestep%analysis_interval == analysis_interval-20)){
                            vA_z = Averages->vn_global(2);
                            muA = rhoA*(tauA-0.5)/3.f;
                            Ca = fabs(vA_z*muA)/(5.796*alpha*1.0);
                            if (Ca == 0) Ca = 1;
                            relError = fabs((Ca - Ca_previous)/Ca);
                                if (rank==0){
                                    printf("timestep=%d Ca = %-15.15E, (previous = %-15.15E, rel error = %-15.15E) \n",timestep,Ca,Ca_previous,relError);
                                    FILE * kr_log_file = fopen("relperm.csv","a+");
                                    fprintf(kr_log_file,"%i %-15.15E %-15.15E\n",timestep,Ca,relError);
                                    fclose(kr_log_file);
                                 }
                                 Ca_previous = Ca;
                     }
