...
double ReadFromBlock( char *ID, int iproc, int jproc, int kproc, int Nx, int Ny, int Nz){
						...
						for ( i=0;i<Nx;i++){
							...
							if (!(x<0) && !(y<0) && !(z<0) && x < 1024  && y < 1024  && z < 1024){
							  ...
							  char value=0;
								...
								//if (id[z*1024*1024 + y*1024 + x] < 215)	value = 1; // CF: Always true because of char comparison
								// set the ID
	...
	printf("Porosity is %.9f \n",porosity);
    
    
    double volume=double(Nx-2)*double(Ny-2)*double(Nz-2);
...
int main(int argc, char **argv)
{
 ...
 //		std::shared_ptr<Domain> Dm (new Domain(nx,ny,nz,rank,nprocx,nprocy,nprocz,Lx,Ly,Lz,BC));
		...
		if (READ_FROM_BLOCK == 0){
            printf("Are we in this READ_FROM_BLOCK section?\n");
			size_t readID;
...
//		MeanFilter(Averages->SDs);
        ...
        for (int k=1; k<nz-1; k++){
            for (int j=1; j<ny-1; j++){
                for (int i=1; i<nx-1; i++){
                    if (Averages->SDs(i,j,k) > 0) count+=1;
                    
                }
            }
        }
        double tmp = double(count);
        MPI_Allreduce(&tmp,&totalGlobal,1,MPI_DOUBLE,MPI_SUM,Dm->Comm);
        
        
        porosity=totalGlobal/volume;
        if (0 == Dm->rank()) printf("porosity = %.15f\n",porosity);
          ...
          for (int k=1; k<nz-1; k++){
              for (int j=1; j<ny-1; j++){
                  for (int i=1; i<nx-1; i++){
                      for (int p=0;p<8;p++){
                      int n = i+cube[p][0] + (j+cube[p][1])*nx + (k+cube[p][2])*nx*ny;
                      if (Dm->id[n] == 2|| Dm->id[n] == 1 )  {  dcount += 0.125; }
                      }
                  }
              }
          }
