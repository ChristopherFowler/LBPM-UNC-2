						...
						for ( i=0;i<Nx;i++){
							...
							if (!(x<0) && !(y<0) && !(z<0) && x < 1024  && y < 1024  && z < 1024){
							  ...
							  char value=0;
								...
								// if (id[z*1024*1024 + y*1024 + x] < 215)	value = 1; CF: This is always true according to the compiler because of char comparison
								// set the ID
...
void lbpm_segmented_pp(int argc, char **argv, int rank, int nprocs, MPI_Comm comm)
{
	        string filename;
    ...
    if (Restart == 0) {
		...
		//.......................................................................
		int nprocx, nprocy, nprocz, nx, ny, nz;
...
//		auto domain_db = db->getDatabase( "Domain" );
 ...
 //		std::shared_ptr<Domain> Dm (new Domain(nx,ny,nz,rank,nprocx,nprocy,nprocz,Lx,Ly,Lz,BC));
		...
		double porosity;
	        ...
	        if (fs == 0) sprintf(LocalRankFilename,"ID.%05i",rank);
		if (fs == 1) sprintf(LocalRankFilename,"%s%s/ID.%05i","/mnt/bb/",usr,rank);
		...
		if (fs == 1) sprintf(LocalRankFilename,"%s%s/SignDist.%05i","/mnt/bb/",usr,rank);
        if (fs == 0) sprintf(LocalRankFilename,"SignDist.%05i",rank);
