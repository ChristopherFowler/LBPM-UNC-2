...
void GenerateSphereTest(int argc, char **argv,int rank, int nprocs, MPI_Comm comm)
{
        string filename;
       filename = argv[1];
 ...
 //   auto domain_db = db->getDatabase( "Domain" );
    ...
    std::string susr;
    int fs = 0;
    susr = domain_db->getScalar<std::string>("usr");
    fs = domain_db->getScalar<int>("filesystem");
   ...
   // int BoundaryCondition=1;
    ...
    if (fs == 1) sprintf(LocalRankFilename,"%s%s%s%s","/mnt/bb/",usr,"/ID.",LocalRankString); // Summit
    if (fs == 0) sprintf(LocalRankFilename,"%s%s","ID.",LocalRankString);  // Promethius
    
    
    if (fs == 1) sprintf(LocalRankFilename,"%s%s%s%s","/mnt/bb/",usr,"/ID.",LocalRankString);
    if (fs == 0) sprintf(LocalRankFilename,"%s%s","ID.",LocalRankString);
    
    
    if (fs == 1)	sprintf(LocalRestartFile,"%s%s%s%s","/mnt/bb/",usr,"/Restart.",LocalRankString);
    if (fs == 0)    sprintf(LocalRestartFile,"%s%s","Restart.",LocalRankString);
    ...
    */
    ...
    if (rank == 0) cout << "post SignedDistance." << endl;
...
for ( k=0;k<2;k++){
    ...
    if (rank==0) printf("Media porosity = %-15.15E \n",porosity);
    ...
    MPI_Allreduce(&sum_local,&porosity,1,MPI_DOUBLE,MPI_SUM,comm);
    porosity = porosity*iVol_global;
    if (rank==0) printf("Media porosity (halo) = %-15.15E \n",porosity);
    ...
    int IS = domain_db->getScalar<int>( "InitialSaturation");
