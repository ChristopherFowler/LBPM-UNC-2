...
void TwoPhase::ComputeVolumeFraction(DoubleArray & Field, DoubleArray & Field_x, DoubleArray& Field_y, DoubleArray& Field_z, DoubleArray& VolumeFraction, bool UpdateNormalToSolid, int rmin, int rmax, int geometry)
{
    ...
    for (int i=1; i<Nx-1; i++) {
        ...
        //...........................................................................
        pmmc_ConstructLocalCube(SDs,  PhaseField,  zero,  zero,  zero, solid_isovalue, fluid_isovalue,
        ...
        if (n_nw_tris > 0 && n_ns_tris == 0) {
            chMyMesh CH_nw;
            nwMyMesh m_nw;

            nwMyMesh::VertexIterator vi;
           

  
            for (int r=0;r<n_nw_tris;r++) {
                D = nw_pts(nw_tris(0,r)); E = nw_pts(nw_tris(1,r)); F = nw_pts(nw_tris(2,r));
                D.z -= 0.5;
                E.z -= 0.5;
                F.z -= 0.5;

                vi = vcg::tri::Allocator<nwMyMesh>::AddVertices(m_nw,3);
                vi->P()=nwMyMesh::CoordType ( D.x, D.y, D.z); ++vi;
                vi->P()=nwMyMesh::CoordType ( E.x, E.y, E.z); ++vi;
                vi->P()=nwMyMesh::CoordType ( F.x, F.y, F.z); ++vi;
            }

           
            for (int p = 0; p < 8; p++) { int m = i+cube[p][0] + (j+cube[p][1])*Nx + (k+cube[p][2])*Nx*Ny;
                if ( SDs(m) > 0 &&  PhaseField(m) > 0 ) {
                    ...
                    vi = vcg::tri::Allocator<nwMyMesh>::AddVertices(m_nw,1);
                    vi->P()=nwMyMesh::CoordType ( Q.x, Q.y, Q.z); vi++;
                }
            }
//            vcg::tri::Clean<nwMyMesh>::RemoveDuplicateFace(m_nw);
//            vcg::tri::Clean<nwMyMesh>::RemoveDuplicateVertex(m_nw,false);

           
            vcg::tri::ConvexHull<nwMyMesh,chMyMesh>::ComputeConvexHull(m_nw,CH_nw);
            PhaseVolume(i,j,k) = vcg::tri::Stat<chMyMesh>::ComputeMeshVolume(CH_nw);

 
            vcg::tri::Append<visMyMesh, chMyMesh>::Mesh(vis_n, CH_nw, false selected , false adjFlag );

            CH_nw.Clear();
            m_nw.Clear();
         }
        else if (n_ns_tris > 0 && n_nw_tris == 0) {
            chMyMesh CH_ns;
            nsMyMesh m_ns;
          
            nsMyMesh::VertexIterator vi;
         
            for (int r=0;r<n_ns_tris;r++) {
                D = ns_pts(ns_tris(0,r)); E = ns_pts(ns_tris(1,r)); F = ns_pts(ns_tris(2,r));
                
                vi = vcg::tri::Allocator<nsMyMesh>::AddVertices(m_ns,3);
                vi->P()=nsMyMesh::CoordType ( D.x, D.y, D.z); ++vi;
                vi->P()=nsMyMesh::CoordType ( E.x, E.y, E.z); ++vi;
                vi->P()=nsMyMesh::CoordType ( F.x, F.y, F.z); ++vi;
            }
            ...
            for (int p = 0; p < 8; p++) {
              int m = i+cube[p][0] + (j+cube[p][1])*Nx + (k+cube[p][2])*Nx*Ny;
                if ( SDs(m) > 0 &&  PhaseField(m) > 0 ) {
                  z = m/(Nx*Ny); y = (m-Nx*Ny*z)/Nx; x = m-Nx*Ny*z-Nx*y;
                  Q.z = z; Q.y = y; Q.x = x;
                    vi = vcg::tri::Allocator<nsMyMesh>::AddVertices(m_ns,1);
                    vi->P()=nsMyMesh::CoordType ( Q.x, Q.y, Q.z); vi++;
                }
            }
            ...
            vcg::tri::ConvexHull<nsMyMesh,chMyMesh>::ComputeConvexHull(m_ns,CH_ns);
            PhaseVolume(i,j,k) = vcg::tri::Stat<chMyMesh>::ComputeMeshVolume(CH_ns);
          
            vcg::tri::Append<visMyMesh, chMyMesh>::Mesh(vis_n, CH_ns, false , false );
   
            CH_ns.Clear(); m_ns.Clear();
        }
        else if (n_ns_tris > 0 && n_nw_tris > 0) {
...
//                D.x += 1.0*Dm->iproc()*(Nx-2); E.x += 1.0*Dm->iproc()*(Nx-2); F.x += 1.0*Dm->iproc()*(Nx-2);
//                D.y += 1.0*Dm->jproc()*(Nx-2); E.y += 1.0*Dm->jproc()*(Nx-2); F.y += 1.0*Dm->jproc()*(Nx-2);
//                D.z += 1.0*Dm->kproc()*(Nx-2); E.z += 1.0*Dm->kproc()*(Nx-2); F.z += 1.0*Dm->kproc()*(Nx-2);

                vi = vcg::tri::Allocator<nsMyMesh>::AddVertices(m_nws,3);
                vi->P()=nsMyMesh::CoordType ( D.x, D.y, D.z); ++vi;
                vi->P()=nsMyMesh::CoordType ( E.x, E.y, E.z); ++vi;
                vi->P()=nsMyMesh::CoordType ( F.x, F.y, F.z); ++vi;
            }
...
//                D.z += 1.0*Dm->kproc()*(Nx-2); E.z += 1.0*Dm->kproc()*(Nx-2); F.z += 1.0*Dm->kproc()*(Nx-2);
            ...
            for (int p = 0; p < 8; p++) {
                ...
                if ( SDs(m) > 0 &&  PhaseField(m) > 0) {
                    ...
                    vi = vcg::tri::Allocator<nsMyMesh>::AddVertices(m_nws,1);
                    vi->P()=nsMyMesh::CoordType ( Q.x, Q.y, Q.z); vi++;
                }
            }
            vcg::tri::Clean<nsMyMesh>::RemoveDuplicateFace(m_nws);
            vcg::tri::Clean<nsMyMesh>::RemoveDuplicateVertex(m_nws,false);

           
            vcg::tri::ConvexHull<nsMyMesh,chMyMesh>::ComputeConvexHull(m_nws,CH_nws);
            PhaseVolume(i,j,k) = vcg::tri::Stat<chMyMesh>::ComputeMeshVolume(CH_nws);

            CH_nws.Clear();
            m_nws.Clear();

           
            vcg::tri::Append<visMyMesh, chMyMesh>::Mesh(vis_n, CH_nws, false , false );
        }
        else {
            ...
            for (int p = 0; p < 8; p++) {
                ...
                if ( SDs(m) > 0 &&  PhaseField(m) > 0) {
                    ...
                    vi = vcg::tri::Allocator<wsMyMesh>::AddVertices(m_n_bulk,1);
                    vi->P()=wsMyMesh::CoordType ( Q.x, Q.y, Q.z); vi++;
                    // PhaseVolume(i,j,k) += 1.0;
                }
            }
            
            // Generate convex hull mesh
            vcg::tri::ConvexHull<wsMyMesh,chMyMesh>::ComputeConvexHull(m_n_bulk,CH_n_bulk);
            PhaseVolume(i,j,k) =  vcg::tri::Stat<chMyMesh>::ComputeMeshVolume(CH_n_bulk);
            
            vcg::tri::Append<visMyMesh, chMyMesh>::Mesh(vis_n, CH_n_bulk,
                                                        false  , false );
...
//                                        vcg::tri::Allocator<nwRootMesh>::AddFace(m_n,nwRootMesh::CoordType ( A.x, A.y, A.z),nwRootMesh::CoordType ( B.x, B.y, B.z),nwRootMesh::CoordType ( C.x, C.y, C.z));
    ...
    sprintf(LocalRankString,"%05d",Dm->rank());
    sprintf(LocalRankFilename,"%s%s%s%s",OffsetString,"_nwsdata.",LocalRankString,".off");
