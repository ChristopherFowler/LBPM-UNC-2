    ...
    const std::array<bool,3>& periodic, const std::array<double,3>& dx )
{
    ...
    fillHalo<int> fillData(  Dm.Comm, Dm.rank_info, n, {1,1,1}, 500, 1, {true,true,true}, periodic );
    ...
    const std::array<bool,3>& periodic, const std::array<double,3>& dx )
{
    ...
    fillHalo<int> fillDataID(  Dm.Comm, Dm.rank_info, n, {1,1,1}, 5000, 1, {true,true,true}, periodic );
    ...
    fillHalo<Vec> fillData( Dm.Comm, Dm.rank_info, n, {1,1,1}, 5000, 1, {true,true,true}, periodic );
    // Calculate the local distances
    calcVecInitialize( d, ID, dx[0], dx[1], dx[2] );
    double err = 1e100;
    double tol = 0.00001 * std::min( std::min(dx[0],dx[1]), dx[2] );
    for (int it=0; it<=10000 && err>tol; it++) {
