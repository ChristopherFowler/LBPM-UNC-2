		...
		int iproc, int jproc, int kproc, int nprocx, int nprocy, int nprocz)
{
    ...
    for (n=0;n<N;n++){ Distance[n]=100.0; }
	//............................................
...
void SignedDistanceLIBB(double *Distance, int nspheres, double *List_cx, double *List_cy, double *List_cz, double *List_rad,
        double Lx, double Ly, double Lz, int Nx, int Ny, int Nz,
        int iproc, int jproc, int kproc, int nprocx, int nprocy, int nprocz)
{
    // Use sphere lists to determine which nodes are in porespace
    ...
    //............................................
    // Lattice spacing
    hx = Lx/((Nx-2)*nprocx-1);  hy = Ly/((Ny-2)*nprocy-1);  hz = Lz/((Nz-2)*nprocz-1);
    //............................................
    // Get maximum and minimum for this MPI process domain
    // Halo is included !
    min_x = double(iproc*(Nx-2)-1)*hx;  min_y = double(jproc*(Ny-2)-1)*hy;  min_z = double(kproc*(Nz-2)-1)*hz;
    //............................................
    
    double rlb = 0;
    double cxlb, cylb, czlb;
    
    double q1,q2;
    
    for (n=0;n<N;n++){ Distance[n]=10000.0; }
    // .........Loop over the spheres.............
    for (p=0;p<5;p++){
        // Get the sphere from the list, map to local min
        cx = List_cx[p] - min_x;
        ...
        /* STEP ONE */
        cout << cx << " " << cxlb << " " << cy << " " << cylb << " " << cz << " " << czlb << " " << r << " " << rlb << " ";
        cout << ceil(rlb) << " " << floor(rlb) << " | ";
        q1 = ceil(rlb) - rlb;
        q2 = rlb - floor(rlb);
        cout << "q1=" << q1 << " q2=" << q2 << " ";
        
        /* STEP TWO */
        // distance = sqrt(cxlb*cxlb + cylb*cylb + czlb*czlb) - rlb;
        //cout << distance << endl;
        
        // Check if
        // Range for this sphere in global indexing
        imin =  ((cx-2*r)/hx);  imax =  ((cx+2*r)/hx)+2;
        jmin =  ((cy-2*r)/hy);  jmax =  ((cy+2*r)/hy)+2;
        kmin =  ((cz-2*r)/hz);  kmax =  ((cz+2*r)/hz)+2;
        ...
        for (i=imin;i<imax;i++){
            for (j=jmin;j<jmax;j++){
                for (k=kmin;k<kmax;k++){
                   ...
                   // if ((std::ceil(x))-x < rlb && x+hx >= rlb) { distance = hx;}
                    // i,j,k are lattice units
                    // if inside sphere, set to zero
                    // get the position in the list
                    n = k*Nx*Ny+j*Nx+i;
                    // Compute the distance
//                    distance = sqrt((cxlb-xlb)*(cxlb-xlb)+(cylb-ylb)*(cylb-ylb)+(czlb-zlb)*(czlb-zlb)) - rlb;
//                    if (abs(distance-0.5*hx)/(r*hx) < 1) distance = hx;
//                    else distance = 0.0;
                    // else distance = 0;
